...
const EstimateForm = ({ estimateId, readOnly = false }) => {
  ...
  useEffect(() => {
    const fetchEstimate = async () => {
      try {
        ...
        console.log("견적서 정보 조회 중 오류 (무시됨):", err);
        if (!readOnly) {
  ...
  const getDealStatusBadge = (status) => {
    const statusMap = {
      1: { text: "작성중", variant: "secondary" },
      2: { text: "송부완료", variant: "primary" },
      3: { text: "검토중", variant: "warning" },
      4: { text: "승인완료", variant: "success" },
      5: { text: "거절됨", variant: "danger" },
    };
